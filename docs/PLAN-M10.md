# kiki: Archive — M10, M10.5, M10.6

This document is the archived implementation log for milestones M10,
M10.5, and M10.6 (mutable pointers, op-heads store, op-store contents).
The main roadmap lives in [`PLAN.md`](./PLAN.md); this file preserves
the detail behind each milestone (trait designs, wire protocols,
decisions, test coverage, outcome records) for spelunking purposes.
**Do not extend; new work goes in `PLAN.md`.**

Section numbers are preserved from PLAN.md as it existed at the end
of M10.6. Cross-references inside this file use these historical
...
- **`gc` on the remote.** The delegate's `gc` cleans up local
  ...
  work.
- **Async push queue + offline resilience.** [`M11-PUSH-QUEUE.md`](./M11-PUSH-QUEUE.md).
